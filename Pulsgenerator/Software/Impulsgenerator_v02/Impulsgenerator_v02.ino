...
ISR(TIMER0_COMPA_vect) {
  ...
  // Rampe Generator
        ...
        dauer = dauer + (potiValueDauer/20); //nach oben zählen
  
      if (dauer <= MinDauer)
        {
        dauer = MinDauer; // nicht unter MinDauer 
        
        }
      if (dauer >= 1023)
        {
        dauer = MinDauer; // Rücksetzen
        } 

       if (potiValueDauer <= 1)
        {
        dauer = MinDauer; // wenn DauerValue Wert zu klein dauer Rücksetzen, sonst keine Ändeurng des Dauer Wertes  
        } 
...
void setIntense() 
  ...
  potiFloat = potiValueIntense;
  puls = dauer*(potiFloat/100);
 // Serial.println(dauer);
 // Serial.println(potiFloat/100);
...
void ladeWerte()
  {
  potiValueIntense = analogRead(potiPinIntense);   
  potiValueIntense = map(potiValueIntense, 0, 1023, 100, 0);
  
  potiValueDauer = analogRead(potiPinDauer);
  potiValueDauer = map(potiValueDauer, 0, 1023, 1023, 0);
  }
